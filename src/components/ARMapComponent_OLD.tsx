...
export default ARMapComponent;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
        ...
        <TabsContent value="streetview" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Camera className="h-5 w-5" />
                Mapillary Street View - {selectedLocation.name}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="flex gap-2">
                  <Button 
                    onClick={initializeStreetView} 
                    disabled={isLoadingMapillary}
                    className="flex items-center gap-2"
                  >
                    <Image className="h-4 w-4" />
                    {isLoadingMapillary ? "Loading..." : "Load Mapillary Images"}
                  </Button>
                  {isStreetViewActive && (
                    <Button onClick={closeStreetView} variant="outline">
                      Close Street View
                ...
                <div className="relative">
                  {!isStreetViewActive ? (
                    <div className="h-96 bg-muted rounded-lg flex items-center justify-center">
                      <div className="text-center">
                        {isLoadingMapillary ? (
                          <>
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-2"></div>
                            <p className="text-muted-foreground">Loading Mapillary images...</p>
                          </>
                        ) : (
                          <>
                            <Image className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                            <p className="text-muted-foreground mb-2">Mapillary Street View Ready</p>
                            <p className="text-xs text-muted-foreground">
                              Click "Load Mapillary Images" to view street-level imagery of {selectedLocation.name}
                            </p>
                          </>
                        )}
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="relative h-96 rounded-lg overflow-hidden border">
                        <img
                          src={mapillaryImages[currentImageIndex].thumb_2048_url}
                          alt={`Street view of ${selectedLocation.name}`}
                          className="w-full h-full object-cover"
                          onError={(e) => {
                            // Fallback to smaller image if 2048 fails
                            const target = e.target as HTMLImageElement;
                            if (target.src !== mapillaryImages[currentImageIndex].thumb_1024_url) {
                              target.src = mapillaryImages[currentImageIndex].thumb_1024_url;
                            }
                          }}
                        />
                        
                        {/* Navigation Controls */}
                        {mapillaryImages.length > 1 && (
                          <>
                            <Button
                              variant="outline"
                              size="icon"
                            ...
                            <Button
                              ...
                              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-background/80 backdrop-blur-sm"
                              onClick={nextImage}
                              disabled={currentImageIndex === mapillaryImages.length - 1}
                            >
                              <ArrowRight className="h-4 w-4" />
                            </Button>
                            ...
                            {/* Image Counter */}
                            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-background/80 backdrop-blur-sm px-3 py-1 rounded-full text-sm">
                              {currentImageIndex + 1} / {mapillaryImages.length}
                            </div>
                          </>
                        )}
                      </div>
                      ...
                      {mapillaryImages.length > 1 && (
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {mapillaryImages.map((image, index) => (
                            <div
                              key={image.id}
                              className={`flex-shrink-0 w-20 h-16 rounded border-2 cursor-pointer overflow-hidden ${
                                index === currentImageIndex ? 'border-primary' : 'border-border'
                    ...
                    <div className="h-96 bg-muted rounded-lg flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-muted-foreground">
                          No Mapillary street view images available for this location
                        </p>
                      </div>
                    </div>
                  )}
                ...
                {isStreetViewActive && mapillaryImages.length > 0 && (
                  <div className="bg-primary/10 p-4 rounded-lg">
                    <h4 className="font-semibold mb-2">Mapillary Street View Controls</h4>
                    <ul className="text-sm text-muted-foreground space-y-1">
                      <li>• Use arrow buttons to navigate between street view images</li>
                      <li>• Click thumbnails to jump to specific viewpoints</li>
                      <li>• Images are captured by Mapillary contributors</li>
                      <li>• High-resolution images provide detailed views of the location</li>
                    </ul>
                  </div>
                )}
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>AR Enhanced Features with Mapillary</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-primary/5 rounded-lg">
              <Image className="h-8 w-8 text-primary mx-auto mb-2" />
              <h4 className="font-semibold">Street-Level Imagery</h4>
              <p className="text-sm text-muted-foreground">
                Real photos from Mapillary contributors
              </p>
            </div>
            <div className="text-center p-4 bg-primary/5 rounded-lg">
              <Navigation className="h-8 w-8 text-primary mx-auto mb-2" />
              <h4 className="font-semibold">Multiple Viewpoints</h4>
              <p className="text-sm text-muted-foreground">
                Browse different angles and perspectives
              </p>
            </div>
            <div className="text-center p-4 bg-primary/5 rounded-lg">
              <MapPin className="h-8 w-8 text-primary mx-auto mb-2" />
              ...
              <p className="text-sm text-muted-foreground">
                Google Places integration with Mapillary imagery
              </p>
            </div>
          </div>
        </CardContent>
